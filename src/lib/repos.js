...
module.exports = {
    ...
    pullRequestAll: (token, type, authed_user, list, callback) => {
        ...
        var hexstring = crypto.randomBytes(Math.ceil(12 / 2)).toString('hex').slice(0, 12);
        var branch = `cider-${hexstring}`;
    ...
    forkExists: (reponame, token, authed_user, callback) => {
        ...
        github.repos.getForks({
            ...
            if (err) {
                ...
                var full_name_array = _.map(data_arr, 'full_name');
                if (err) {
                    log(err);
                    return callback(false);
